...
let max = function(role, deep, totalDeep) {
  debugNodeCount++;
  //board.logSteps();
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    let m = min(R.reverse(role), deep - 1);
    board.remove(p);
    if (m) {
      if (m.length) {
...
let min = function(role, deep) {
  ...
  if (deep <= 1) return false;
  let points = findMin(role, MIN_SCORE);
  if (points.length === 0) return false;
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    board.put(p, role);
    lastMinPoint = p;
    let m = max(R.reverse(role), deep - 1);
    board.remove(p);
    if (m) {
      m.unshift(p);
      cands.push(m);
...
//迭代加深
let deeping = function(role, deep, totalDeep) {
  let start = new Date();
  debugNodeCount = 0;
  for (let i = 1; i <= deep; i++) {
    lastMaxPoint = undefined;
    lastMinPoint = undefined;
    let result = max(role, i, deep);
...
let vcx = function(role, deep, onlyFour) {
  ...
  if (onlyFour) {
    ...
    let result = deeping(role, deep, deep);
    if (result) {
      result.score = S.FOUR;
      return result;
    }
...
// 连续冲四
let vcf = function(role, deep) {
  let c = getCache(true);
  if (c) return c;
  let result = vcx(role, deep, true);
  cache(result, true);
  return result;
};

// 连续活三
let vct = function(role, deep) {
  let c = getCache();
  if (c) return c;
  let result = vcx(role, deep, false);
  cache(result);
  return result;
};
