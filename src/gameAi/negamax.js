 ...
 */
let negamax = function(candidates, role, deep, alpha, beta) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    board.put(p, role);
    let steps = [p];
    let v = r(deep - 1, -beta, -alpha, R.reverse(role), 1, steps.slice(0), 0);
    v.score *= -1;
...
let r = function(deep, alpha, beta, role, step, steps, spread) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    _steps.push(p);
    let v = r(_deep, -beta, -alpha, R.reverse(role), step + 1, _steps, _spread);
    v.score *= -1;
    board.remove(p);
...
let deeping = function(candidates, role, deep) {
  start = (+new Date());
...
let deepAll = function(role, deep) {
  role = role || R.com;
