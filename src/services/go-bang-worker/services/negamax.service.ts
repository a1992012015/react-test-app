...
export class Negamax {
    ...
    AI.log &&
      console.log(
        candidates.map((d) => {
          return (
            `[${d.x},${d.y}]` +
            `,score:${d.data?.score},step:${d.data?.step},steps:${d.data?.steps?.join(';')}${
  ...
  deepen = (candidates: Piece[], role: ERole, deep: number): IPiece => {
    ...
    if (AI.log) {
      ...
      console.log(
        `选择节点: [${result.x}, ${result.y}], 分数: ${result.score.toFixed(3)}, 步数: ${
          result.step
        }, 最小值：${result.min}`
      );
      console.log(
      ...
      console.log(
        `搜索缓存: 总数 ${this.cacheCount}, 命中率 ${(
          (this.cacheGet / this.cacheCount) *
          100
        ).toFixed(3)}%, ${this.cacheGet}/${this.cacheCount}`
      );
      // 注意，减掉的节点数实际远远不止 ABcut 个，因为减掉的节点的子节点都没算进去。实际 4W个节点的时候，剪掉了大概 16W个节点
      console.log(
        `当前统计: ${this.count}个节点, 耗时: ${time.toFixed(2)}s, NPS: ${Math.floor(
          this.count / time
        )}N/S`
      );
      console.log('===============统计表===============');
    ...
    AI.debug && statistic.print(candidate);
  ...
  search = (data: ISearch): IScore => {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      let nowSpread = spread;

      if (nowSpread < AI.spreadLimit) {
        // 冲四延伸
        const { scoreCom } = p;
        const { scoreHum } = p;
      ...
      const nowSteps = cloneDeep(steps);
      nowSteps.push(p);
      const searchData = {
        deep: nowDeep,
        beta: -beta,
