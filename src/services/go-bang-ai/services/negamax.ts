...
import { equal, greatOrEqualThan, littleOrEqualThan } from './math';
 ...
 */
const negamax = (candidates: Piece[], role: Role, deep: number, alpha: number, beta: number) => {
  count = 0;
  ABcut = 0;
  PVcut = 0;
  board.currentSteps = [];
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    board.put(p, role);
    const steps = [p];
    const v = r(deep - 1, -beta, -alpha, aiRole.reverse(role), 1, steps.slice(0), 0);
    v.score *= -1;
  ...
  aiConfig.log && console.log(candidates.map((d) => {
    return '[' + d.x + ',' + d.y + ']'
      + ',score:' + d.v?.score
      + ',step:' + d.v?.step
...
const r = (deep: number, alpha: number, beta: number, role: Role, step: number, steps: Piece[], spread: number): Score => {
  ...
  if (aiConfig.cache) {
    ...
    if (c) {
      if (c.deep >= deep) { // 如果缓存中的结果搜索深度不比当前小，则结果完全可用
        ...
        // TODO: 存疑这个位置的写法有待验证
        if (greatOrEqualThan(c.score.score, SCORE.FOUR) ||
  ...
  aiConfig.debug &&
  console.log('points:' + points.map((d) => '[' + d.x + ',' + d.y + ']').join(','));
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (_spread < aiConfig.spreadLimit) {
      // 冲四延伸
      const scoreCom = p?.scoreCom || 0;
      const scoreHum = p?.scoreHum || 0;
      if ((role === aiRole.com && scoreHum >= SCORE.FIVE) ||
      ...
      // 单步延伸策略：双三延伸
      // if ( (role == aiRole.com && p.scoreCom >= SCORE.THREE * 2) || (role == aiRole.hum && p.scoreHum >= SCORE.THREE*2)) {
      //  _deep = deep
      //  _spread ++
      // }
    }

    const _steps = steps.slice(0);
    _steps.push(p);
    const v = r(_deep, -beta, -alpha, aiRole.reverse(role), step + 1, _steps, _spread);
    v.score *= -1;
    board.remove(p);
...
const cache = (deep: number, score: Score) => {
  if (!aiConfig.cache) {
    return false;
  }
  if (score.abcut) {
...
const deeping = (candidates: Piece[], role: Role, deep: number) => {
  start = (+new Date());
  ...
  for (let i = 2; i <= deep; i += 2) {
    ...
    // if (littleThan(bestScore, T.THREE * 2)) bestScore = MIN // 如果能找到双三以上的棋，则保留bestScore做剪枝，否则直接设置为最小值
  }
  ...
  aiConfig.log && console.log(
    '选择节点：' + candidates[0] + ', 分数:' + result.score?.toFixed(3) + ', 步数:' + result.step +
    ', 最小值：' +
    result.min);
  const time = (+new Date() - start) / 1000;
  aiConfig.log && console.log('搜索节点数:' + count + ',AB剪枝次数:' + ABcut + ', PV剪枝次数:' + PVcut);
  aiConfig.log && console.log(
    '搜索缓存:' + '总数 ' + cacheCount + ', 命中率 ' + (cacheGet / cacheCount * 100).toFixed(3) + '%, ' +
    cacheGet + '/' + cacheCount);
