...
export class Board {
  ...
  initScore = () => {
    ...
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[i].length; j++) {
        ...
        } else if (board[i][j].role === aiRole.com) { // 对电脑打分，玩家此位置分数为0
          this.comScore[i][j].score = scorePoint(this, i, j, aiRole.com);
          this.humScore[i][j].score = 0;
        } else if (board[i][j].role === aiRole.hum) { // 对玩家打分，电脑位置分数为0
  ...
  evaluate(role: Role) {
    ...
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[i].length; j++) {
        if (board[i][j].role === aiRole.com) {
          this.comMaxScore += fixScore(this.comScore[i][j].score || 0);
        } else if (board[i][j].role === aiRole.hum) {
          this.humMaxScore += fixScore(this.humScore[i][j].score || 0);
        }
      }
    }
    // 有冲四延伸了，不需要专门处理冲四活三
  ...
  gen(role: Role, onlyThrees?: boolean, starSpread?: boolean): Piece[] {
    ...
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board.length; j++) {
        if (board[i][j].role === aiRole.empty) {
          ...
          if (onlyThrees && maxScore < SCORE.THREE) continue;
    ...
    const blockedfours = role === aiRole.com ?
  ...
  update = (x: number, y: number, dir: number) => {
    const role = this.board[x][y].role;
    if (role !== aiRole.reverse(aiRole.com)) {
      this.mergeScore(x, y, dir, aiRole.com);
    } else if (role !== aiRole.reverse(aiRole.hum)) {
      this.mergeScore(x, y, dir, aiRole.hum);
    } else {
      this.comScore[x][y].score = 0;
    }
  };

  mergeScore = (x: number, y: number, dir: number, role: Role) => {
    const cs = scorePoint(this, x, y, role, dir);
    this.comScore[x][y].score = cs;
    statistic.table[x][y].score = (statistic.table[x][y].score || 0) + cs;
  };
