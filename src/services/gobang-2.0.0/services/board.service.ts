...
export class Board extends FilterCandidates {
   ...
   * @param role 当前的选手
   */
  protected evaluate = (role: ERole): number => {
    ...
    return (role === this.playChess ? 1 : -1) * (whiteMaxScore - blackMaxScore);
  };
  ...
  private doubleStarExtension = (p: IPiece, scores: IScoreElement, star?: boolean): void => {
    ...
    if (star && AI.star) {
      ...
      const scoreWhite = this.scoreWhite[y][x];
      const maxScore = Math.max(scoreBlack, scoreWhite);
      // const roleScore = role === ERole.white ? p.scoreCom : p.scoreHum;
      // const deRoleScore = role === ERole.white ? p.scoreHum : p.scoreCom;
