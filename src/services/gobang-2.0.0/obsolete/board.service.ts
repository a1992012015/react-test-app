...
import { IOpen } from '../interfaces/board.interface';
...
export class Board {
  ...
  init = (board: IOpen, first: boolean): IOpen => {
    ...
    if (board.board.length === 15 && board.board.every((b) => b.length === 15)) {
    ...
    for (let i = 0; i < this.board.board.length; i++) {
      this.count += this.board.board[i].filter((d) => d.role > ERole.empty).length;
    }

    const size = this.board.board.length;
  ...
  put = (piece: IPiece): void => {
    ...
    this.board.board[piece.y][piece.x] = piece;
  ...
  remove = (p: IPiece): void => {
    zobrist.go(p);
    this.board.board[p.y][p.x].role = ERole.empty;
    // 移除之后也需要更新周围的分数
    this.updateScore(p);
    this.allSteps.pop();
  ...
  initScore = (): void => {
    ...
    for (let y = 0; y < board.length; y++) {
      for (let x = 0; x < board[y].length; x++) {
        ...
        } else if (board[y][x].role === ERole.white) {
          // 对电脑打分，玩家此位置分数为0
    ...
    for (let py = startY; py <= endY; py++) {
      ...
      if (py >= 0 && py < len) {
        for (let px = startX; px <= endX; px++) {
          ...
          if (px >= 0 && px < len) {
            // 需要计算的点，所以不需要检测
            if ((py !== y || px !== x) && board[py][px].role !== ERole.empty) {
              // 不是自身棋子，有其他棋子的位置
              nowCount--;
              // 检测到足够的邻居返回
              if (nowCount <= 0) {
   ...
   * @param role 当前的选手
   */
  evaluate = (role: ERole): { evaluate: number; comScore: number; humScore: number } => {
    // 这里加了缓存，但是并没有提升速度
    ...
    const evaluate = (role === ERole.white ? 1 : -1) * (comMaxScore - humMaxScore);
    return { evaluate, comScore: comMaxScore, humScore: humMaxScore };
  };
  ...
  updateScore = (p: IPiece): void => {
    const radius = 4;
    const len = this.board.board.length;
  ...
  update = (x: number, y: number, dir: number): void => {
    const { role } = this.board.board[y][x];
    const data = {
      x,
      y,
      dir,
  ...
  mergeScore = (data: IScorePoint, role: ERole, scores: number[][]): void => {
    if (role !== commons.reverseRole(role)) {
      // 空位和自己的棋子才更新分数
  ...
  toString = (): string => {
    return this.board.board.map((d) => d.map((p) => `[${p.y}, ${p.x}]`).join(';')).join('\n');
  };
}
