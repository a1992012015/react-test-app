  ...
  for (let i = 0; i < board.board.length; i++) {
    for (let j = 0; j < board.board[i].length; j++) {
      if (board.board[i][j].role === aiRole.empty) {
        ...
        const s1 =
          (role === aiRole.com ? board.comScore[p.x][p.y].score : board.humScore[p.x][p.y].score) ||
          0;
        const s2 =
          (role === aiRole.com ? board.humScore[p.x][p.y].score : board.comScore[p.x][p.y].score) ||
          0;
        if (s1 >= SCORE.FIVE) {
          p.score = -s1;
          return [p];
...
const cache = (result: Piece | boolean, vcf = false) => {
  if (!aiConfig.cache) {
    return;
  }
